...
public class EmployeeService implements EmployeeServiceInterface{
	...
	public Employee addEmployee(Employee employee) { 
			if (employee.getName().isEmpty() || employee.getName().length() == 0) {
				throw new BusinessException("601 ","Please send proper, It blank");	
			}
		 try {
	...
	public List<Employee> getAllEmployees() {
		...
		if (empList.isEmpty()) 
			throw new BusinessException("604","Het list is completely empty, we have nothing to return");
			return empList;
		}
	...
	public Employee getEmpById(Long empidL)  {
		try {
			...
			throw new BusinessException("606","given employee is null, please send some id to be searched" + e.getMessage());
		}
